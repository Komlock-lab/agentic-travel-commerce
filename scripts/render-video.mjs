...
const outputFile = path.join(outputDir, "agentic-travel-commerce-concept.mp4");
const posterFile = path.join(outputDir, "agentic-travel-commerce-poster.png");

const fps = Number(process.env.VIDEO_FPS || 24);
const duration = Number(process.env.VIDEO_DURATION || 90);
...
for (let frame = 0; frame < frameCount; frame += 1) {
  ...
  await page.evaluate((time) => window.setDemoTime(time), seconds);
